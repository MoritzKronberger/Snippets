...
("id"            UUID                              DEFAULT gen_random_uuid(),
 ...
 CONSTRAINT fk_language_id
    FOREIGN KEY ("language_id") REFERENCES e_language ("id") ON DELETE CASCADE,
...
/* deletes categories that are no longer used by any posts */
...
/* from https://gitlab.multimedia.hs-augsburg.de/kowa/wk_account_postgres_01a.git */
CREATE FUNCTION check_password(usr VARCHAR, pw VARCHAR) RETURNS BOOLEAN AS
$_SQL_$
    SELECT EXISTS
        (SELECT *
         FROM   account
         WHERE  usr = "username" AND "password" = crypt(pw, "password")
        );
$_SQL_$
LANGUAGE SQL
IMMUTABLE
