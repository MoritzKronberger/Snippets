...
CREATE VIEW get_full_comment (id, creation_time, content, post_id, user_id, username, num_likes)
AS
SELECT c.id, c.creation_time, c.content, p.id AS post_id, a.id AS user_id, a.username, COUNT(lk.id) AS num_likes
FROM comment c
